...
console.log("COLOUR FOLLOWS TONE, NOT SYSTEM");
{
  ...
  // The card hands the drawing the tone it computed for itself.
  ok(/const artTone = toneFor\(x\);/.test(HTML), "TONE-10: the card computes its tone once");
  ok(/prioArtSVG\(artSysId, i, artTone\)/.test(HTML), "TONE-11: and passes that same tone to the drawing");
}
...
console.log("THE CARD AND THE CSS");
{
  ...
  ok(/prioArtSVG\(artSysId, i, artTone\)/.test(HTML), "CARD-4: the card renders the drawing from the DRIVER's system");
