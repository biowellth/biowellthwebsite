...
const src = [extractLine("SENSITIVE_SYSTEMS"), extractBlock("PRIO_ART"), extractBlock("CANONICAL_TITLE"),
             extract("prioArtSVG"),
             ";globalThis.__art = prioArtSVG; globalThis.__set = PRIO_ART;",
             ";globalThis.__titles = CANONICAL_TITLE; globalThis.__sens = SENSITIVE_SYSTEMS;"].join("\n");
new Function(src)();
const art = globalThis.__art, SET = globalThis.__set, TITLES = globalThis.__titles, SENS = globalThis.__sens;
...
console.log("ONE DRAWING PER MAPPED SYSTEM");
{
  ...
  ok(art("liver", 2).includes("--pa-i:2"), "ART-6: and it is the card's own index");
}

console.log("NO DRAWING RATHER THAN A GENERIC ONE");
{
  eq(art("no_such_system", 0), "", "ART-7: an unmapped system renders nothing");
  eq(art(null, 0), "", "ART-8: a missing system renders nothing");
  eq(art(undefined, 0), "", "ART-9: undefined renders nothing");
  eq(art("", 0), "", "ART-10: an empty system renders nothing");
}

console.log("SAFETY CLASSES CARRY NO DECORATION");
{
  eq(art("heavy_metals", 0), "", "SAFE-1: a heavy-metals priority renders no drawing");
  eq(art("autoimmune", 0), "", "SAFE-2: an autoimmune priority renders no drawing");
  eq(art("tumor_markers", 0), "", "SAFE-3: a tumour-marker priority renders no drawing");
  ...
  ok(artCount(art("thyroid", 1)) === 1, "SAFE-4: an ordinary priority beside them still draws");
  for (const id of SENS) ok(art(id, 0) === "", "SAFE-5." + id + ": every id in SENSITIVE_SYSTEMS draws nothing");
...
console.log("THE SET ITSELF");
{
  ...
  for (const k of keys) {
    const a = SET[k];
    ok(Array.isArray(a.d) && a.d.length > 0, "SET-5." + k + ": has at least one path");
    ok(typeof a.stroke === "string" && a.stroke.indexOf("var(--") === 0,
       "SET-6." + k + ": strokes with an existing token, never a raw hex");
    ok(typeof a.tint === "string" && a.tint.indexOf("var(--") === 0,
       "SET-7." + k + ": tints with an existing token, never a raw hex");
  }
}

console.log("THE CARD AND THE CSS");
{
  const prioRule = /^\.prio\{[^}]*\}/m.exec(HTML);
  ok(!!prioRule, "CARD-1: the .prio rule is still in the file");
  ok(prioRule[0].indexOf("border-left") === -1,
     "CARD-2: and it no longer carries the coral left border  (" + prioRule[0].slice(0, 70) + ")");
  // Scoped to the DASHBOARD card's own tone rules. #view-reveal .prio-card at :993-995 is a
  // different component with a live left border, and asserting against the whole file would have
  // demanded a change there that this pass was told not to make.
  ok(!/^\.prio\.t-[a-z]+\{border-left-color/m.test(HTML),
     "CARD-3: no .prio tone rule is left colouring a border that no longer exists");
  ok(/#view-reveal \.prio-card\.t-coral\{border-left-color/.test(HTML),
     "CARD-3b: and the reveal deck's own cards are untouched, so CARD-3 is scoped not vacuous");
  ok(/prioArtSVG\(artSysId, i\)/.test(HTML), "CARD-4: the card renders the drawing from the DRIVER's system");
  ok(/const artSysId = pm \? chipSysByMarker\[pm\.marker_id\] : null;/.test(HTML),
     "CARD-5: resolved through chipSysByMarker, never priority.system_id");
  ok(!/prioArtSVG\(x\.system_id/.test(HTML), "CARD-6: priority.system_id is not the key anywhere");
  ok(HTML.indexOf('.prio-art path{fill:none;stroke-width:1.4') > -1, "CARD-7: one line weight, 1.4");
  ok(/@media\(max-width:480px\)\{\.prio-art\{width:56px;height:56px\}/.test(HTML),
     "CARD-8: the circle shrinks to 56px under 480, and the row stays horizontal");
  ok(/\.pa-anim \.prio-art path\{stroke-dasharray/.test(HTML), "CARD-9: the draw-in is a dashoffset animation");
  ok(/animation-delay:calc\(var\(--pa-i,0\) \* \.3s\)/.test(HTML), "CARD-10: staggered 0.3s per card");
  ok(/if\(!PRIO_ART_DRAWN\)\{ PRIO_ART_DRAWN = true;/.test(HTML), "CARD-11: once per page life, not once per render");
  const rm = /@media\(prefers-reduced-motion:reduce\)\{[\s\S]*?\n\}/.exec(HTML);
  ok(!!rm && rm[0].indexOf(".prio-art path") > -1,
     "CARD-12: the existing reduced-motion block covers the drawings");
  ok(!!rm && /\.prio-art path\{animation:none!important;stroke-dasharray:none!important/.test(rm[0]),
     "CARD-13: static AND complete, not frozen at the start offset");
}
